#!/bin/bash
# RULER evaluation with Qwen3-8B-128k using ShadowKV (CPU mode)
# 100k context, 10 tasks (niah_single_3 excluded)
# sparse_budget_ratio=0.27: total KV = 27% of input
...
LOG_DIR="${SHADOWKV_DIR}/logs/Qwen3-8B-128k_ruler_shadowkv_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

echo "=== RULER ShadowKV ===" | tee "$LOG_DIR/run.log"
